...
export function MapView({ stakeholders, scores, team, updateCoordForStakeholder, selectedId, setSelectedId, openDetail, tweaks, users }) {
...
function MapDetail({ stakeholder, onOpenFull, team, scores, stakeholders, setSelectedId, onClose, users, historyMode, onToggleHistory }) {
  ...
  const meta = STAKEHOLDER_DATA.STATUSES[stakeholder._status];
  return (
    <div className="detail">
      <div className="detail-head-row">
        <strong style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--ink-3)" }}>Scorecard</strong>
      ...
      <h3>{displayName(stakeholder) || stakeholder.name}</h3>
      <div className="det-org">{stakeholder.org}</div>
      {stakeholder.url && (
        <div style={{ marginTop: -4, marginBottom: 8 }}>
          <a className="det-website" href={normalizeUrl(stakeholder.url)} target="_blank" rel="noopener noreferrer">
            Visit Website
          </a>
        </div>
      )}
