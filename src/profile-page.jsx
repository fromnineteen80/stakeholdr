...
import { displayName, Icon, FilterSection, SortFieldList, StatusPill, PriorityPill } from './components';
import { cmdKeyLabel } from './store';
import { Avatar, OwnersDisplay, ManagerBadge } from './users';
...
export function ProfilePage({
  ...
  if (tab === "ws") {
    ...
    rows = shAssigned.map(s => ({
      id: s.id, name: displayName(s) || s.name,
  ...
  return (
    <div className="single-page">
          ...
          <div className="profile-controls">
            <div className="search"><Icon name="search" /><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search…" /><span className="kbd kbd-cmdk">{cmdKeyLabel}</span></div>
                  ...
                  {cols.map(c => (
                    <span key={c.key} className="profile-td">
                      {c.key === "owner" ? <OwnersDisplay users={users} owners={r._owners} size={22} />
                        : c.key === "status" ? <span className="comm-stage-text" style={{ color: PLAN_STAGE_FG[r.status] || "var(--ink-2)" }}>{r.status}</span>
                        : r[c.key]}
                    </span>
                  ))}
                </div>
