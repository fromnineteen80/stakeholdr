...
export function SheetView({ explainerSlot, addNonce, addNonceFor, stakeholders, scores, team, updateStakeholder, openDetail, selectedId, setSelectedId, workspaceLabel, isMaster, getWorkspacesForStakeholder, workspaces, users, addStakeholder, openStakeholderId, onConsumeOpen, currentUser, companyIssues, community, updateCommunityApp, onOpenWorkspace }) {
  const { CATEGORIES, weightedCoord, statusFor, STATUS_ORDER } = STAKEHOLDER_DATA;
  ...
  const filtered = useMemo(() => {
    ...
    if (sortKey) {
      out = [...out].sort((a, b) => {
        ...
        if (sortKey === "name")     { av = displayName(a) || a.name || ""; bv = displayName(b) || b.name || ""; }
        ...
        if (sortKey === "site")     { av = STAKEHOLDER_DATA.siteLabel((STAKEHOLDER_DATA.SITES||[]).find(s=>s.id===a.site)); bv = STAKEHOLDER_DATA.siteLabel((STAKEHOLDER_DATA.SITES||[]).find(s=>s.id===b.site)); }
        let cmp = 0;
  ...
  function exportCsv() {
    const cols = [
      ["Stakeholder", r => displayName(r) || r.name || ""],
      ...
      ["Website", r => r.url ? normalizeUrl(r.url) : ""],
      ["Notes", r => r.notes || ""]
    ];
    const esc = (v) => {
    ...
    <div className="sheet-wrap">
      ...
      {notesFor && (
        <NotesModal
          ...
          onAdd={(text) => {
            const entry = { id: uid("n"), body: text, at: new Date().toISOString(), by: currentUser.id };
...
function SheetRow({ row, index, frozenLeft, columns, selected, onSelect, onOpenDetail, onOpenNotes, update, isMaster, getWorkspacesForStakeholder, workspaces, users, community }) {
  const { CATEGORIES } = STAKEHOLDER_DATA;
  ...
  function renderCell(key) {
    switch (key) {
      ...
      case "market":
        return (
          <CellSelect value={row.market} options={Object.keys(STAKEHOLDER_DATA.MARKETS)}
            onChange={m => update({ market: m, region: (STAKEHOLDER_DATA.MARKETS[m] || [])[0] || "" })} />
        );
      case "region":
        return <CellSelect value={row.region} options={STAKEHOLDER_DATA.MARKETS[row.market] || []} onChange={v => setField("region", v)} />;
      case "geography":
        return <CellSelect value={row.geography} options={STAKEHOLDER_DATA.GEOGRAPHIES} onChange={v => setField("geography", v)} />;
      case "state": {
        return <CellSelect value={row.state || ""} placeholder="-" options={STAKEHOLDER_DATA.US_STATES || []} onChange={v => setField("state", v)} />;
      }
      case "site": {
        return (
          <CellSelect value={row.site || ""} placeholder="-"
            options={(STAKEHOLDER_DATA.SITES || []).map(s => ({ value: s.id, label: STAKEHOLDER_DATA.siteLabel(s) }))}
            onChange={id => { const s = (STAKEHOLDER_DATA.SITES || []).find(x => x.id === id); update(s && s.state ? { site: id, state: s.state } : { site: id }); }} />
        );
      }
      ...
      case "phone":
        return row.phone
          ? <a className="plain-link" href={"tel:" + row.phone.replace(/[^\d+]/g, "")} onClick={(e) => e.stopPropagation()}>{formatPhone(row.phone)}</a>
          : <span className="muted">-</span>;
      case "xAccount": {
      ...
      case "url":
        return row.url
          ...
          : <span className="muted">-</span>;
      case "community": {
        const affil = affiliatedCommunity(row.id, community);
...
export function CellSelect({ value, options, onChange, placeholder }) {
...
export function CellDate({ value, onChange }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);
