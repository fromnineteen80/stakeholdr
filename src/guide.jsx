...
• TIER 1 — RECORDS (stakeholders, plans, community, workspaces, users, appConfig): mutate with TARGETED COLUMN-LEVEL upserts of ONLY the changed fields — never the whole array, never a blind whole-row write. Concurrency = optimistic + updated_at/version: if A edits name while B edits email on the same record, column-level writes don't collide; if two edit the SAME field, last-write-wins by updated_at (acceptable + visible). Realtime: postgres_changes → MERGE the incoming row BY ID into the cache.
