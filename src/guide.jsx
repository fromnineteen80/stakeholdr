...
CANONICAL-UI BUILD MAP (NEVER md-*/shadcn) — host = ui-dialog with head title "New stakeholder" / "Edit stakeholder" (Newsreader title type), the "View Stakeholder" ui-button (variant text, ui-tooltip "View full profile") beside it in edit mode, and a close ui-icon-button (icon close, aria-label "Close"). Text inputs (Organization, Website, Email, Phone, X account, person First/Last, custom title) = ui-text-field; selects (Title, Category, Audience type, Market, Region, Geography, Site, State, Priority, Status) = ui-select; Last contact = ui-date-picker (or native date if simpler); "Add a person" = ui-switch (or ui-checkbox). IssueSelector = a ui-chip set (selected filled chips with remove, available dashed add chips) + a ui-text-field for custom entry with placeholder "Add custom issues, separated by commas" (the text field is omitted when restrict). Owners = the owner-avatar picker. Photo = a plain file input behind a ui-button (Upload/Replace) plus a ui-button ghost "Remove"; preview is an avatar surface with the data URL or a fallback ui-icon. Footer = "Required: …" hint + ui-button "Cancel" + primary ui-button labelled "Create stakeholder" / "Save changes" (disabled until valid). The delete-confirm is a second stacked ui-dialog ("Delete this stakeholder?" / "Cancel" + danger "Delete"). Tone, spacing, and colors come only from --ui-sys-* tokens.` },
...
Opens when detailId is set (a stakeholder id). A drawer-veil overlay covers the app (click → setDetailId(null)) and the drawer panel slides in from the right; both gain class "show" while open. detailStakeholder = the enriched stakeholder (with computed _x/_y/_status). A guard effect re-points detailId to the first stakeholder if the current one was deleted (shared with selectedId, see pointers below). BUILD MAP: ui-sheet (right side) or ui-inspector as the host; fields are ui-text-field / ui-select / ui-date-picker / ui-icon-button; every edit writes IMMEDIATELY via updateStakeholder(id, patch) (no save button).
...
deleteStakeholder (removes from stakeholders + deletes scores[id] + stakeholderWorkspaces[id]); updateStakeholder/updateScore/updateTeam/updateWorkspace/removeWorkspace (cascades to stakeholderWorkspaces + plans + open tabs + active fallback)/updateCommunityApp (upsert by id)/updatePlan (upsert by id, one plan per workspace)/deletePlan; messaging sendMessage/startConversation (DM dedupe by participant pair)/messageUser. A guard effect re-points selectedId/detailId to the first stakeholder if the current one was deleted. These are captured fully in the Lists / Scoring / Plans / Community / Messaging boxes; listed here only to show they are wired from the shell. (The detail drawer is NOT a pointer — it is captured in full in the DETAIL DRAWER section above.)

══ CANONICAL UI BUILD MAP ══
ui-app-shell = the whole frame · ui-app-bar = brand bar (Row 1) · ui-tabs = nav-tab row (Row 2) and bottom workspace-tab strip (Row 4) · ui-sidebar = IntelPanel / messaging sidebar hosts · ui-sheet (right) or ui-inspector = the detail drawer · ui-dialog = OpenWorkspaceModal, CommandPalette, EditProfileModal, StakeholderModal, UserListPopup · ui-icon = every glyph (Material Symbols ligatures per the ALIAS→LIGATURE table in the NAV_TABS section — never the raw alias strings) · ui-icon-button = close/create/message/fan-toggle controls. No md-*, no shadcn, no hand-rolled glyphs.` },
...
2. NEED YOUR SCORE (wide) — names = needsScore.slice(0, mode==="intel" ? 12 : 5).map(nameOf); more = needsScore.length - (12 or 5); empty "You're caught up on scoring". (Feeds from the buggy needsScore above — rebuild from the canonical isUnscoredBy, per the DO-NOT-REPLICATE ruling.)
...
• ui-avatar — PRESENCE (the row of teammates currently on the board, live-cursor identity chips) = GAP: no ui-avatar exists in the manifest today. Build it into design-system/ (initials/photo variants, sizes from tokens, stacked-overlap group form), register it in manifest.json, then use it — same GAP procedure as the canvas.
...
STATUS OF THIS BOX — forward-design: requirements are the contract; the concrete ui-whiteboard and ui-avatar component specs, the comment/idea interaction design, the Messages-vs-Whiteboard relationship, and the embed API choice are designed WITH the user before build, then this box is amended with the final ui-* build-map and sealed.` },
...
- MultiOwnerPicker / OwnerPicker → ui-owner-picker (avatar-stack + add-via-autocomplete + arm-then-confirm remove; single-owner mode = the OwnerPicker config: placeholder "Unassigned", clearable, showAvatar) — GAP. OwnersDisplay popover → ui-avatar-stack + ui-menu/popover.
