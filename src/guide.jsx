...
MUI BUILD MAP — matrix: DataGrid (sticky first "Stakeholder" column, virtualized rows, custom cell renderers for editable vs read-only) or a composed Table for the simpler case; editable x/y cell: two TextField type="number" small + stepper IconButtons; weight: Slider (marks, min 0.1/max 2.0/step 0.1); % of team: Chip; teammate card: Card + Avatar + Typography + IconButton; add-teammate: Dialog + Autocomplete; last-teammate: Dialog + Select + destructive Button; relationship: Chip (zone-themed). Stakeholder name cell is a clickable link (ButtonBase) opening that stakeholder's profile.` },
      { t: "Map — coordinate→pixel translation, dots, zones, drag-to-rescore, history trail, scorecard", d:
`WHAT IT IS — the Map is the visual face of the Relationship engine: a 4-column x 6-row zone grid with one draggable DOT per (visible) stakeholder, positioned by that stakeholder's weighted (x, y). It is TWO-WAY: reading shows positions; dragging a dot writes back to the underlying scores. Rows = the same workspace-filtered visibleStakeholders as everywhere else; team + scores are global (see Scoring). Unlike Scoring, the Map IS available on Master (it is the org-wide overview Master redirects Scoring to).
...
Inverse, used when dragging: pctToCoord(left%, top%): x = (left%/100)*20 - 10 ; y = 10 - (top%/100)*20.
...
THE DOT — absolutely positioned via coordToPct(_x, _y). A circular inner marker: background = zone color, text color = zone text, border = zone.border or zone.text; when selected the border becomes the accent color. Shows the stakeholder's 2-letter INITIALS when the dot is large enough (abbrev(name): strip honorific prefixes Mayor/Rep./Sen./Dr./Mr./Ms./Mrs., then first+last initial, or first two letters of a single name). A text LABEL (the name) shows when labels are on OR the dot is selected. Hover tooltip: "{name} · {zone} · ({x.toFixed(1)}, {y.toFixed(1)})". Interactions: single click/pointer-down = select (opens the scorecard, exits history mode); double-click = open the full stakeholder profile.

DRAG TO RESCORE (two-way binding) — on pointer-down a dot enters dragging; on pointer-move the pointer's position within the dots-layer rect becomes left%/top% (clamped 0..100) -> pctToCoord -> a target (x, y), ROUNDED to the nearest 0.25 (Math.round(v*4)/4) for stability; that target is committed via updateCoordForStakeholder(id, targetX, targetY); pointer-up ends the drag.
updateCoordForStakeholder computes the current weighted (x, y), takes the DELTA (dx = targetX - currentX, dy = targetY - currentY), and shifts the scores by that same delta, each clamped to [-10, 10], restamping updatedAt (createdAt preserved). Because the weighted average of a set shifted by a constant moves by exactly that constant, the blended dot lands on the target while the SPREAD of individual opinions is preserved. CORRECTION (per our "unscored != (0,0)" rule): the delta is applied ONLY to teammates who have ALREADY scored this stakeholder, so dragging never fabricates scores for people who haven't weighed in; if NOBODY has scored yet, the drag sets the acting user's OWN score to the dropped coordinate (the one judgment we can attribute). (The old build shifted every teammate including unscored ones defaulted to 0,0, which silently materialized opinions nobody gave.)

HISTORY / MOVEMENT OVER TIME — a stakeholder carries history: an array of past snapshots, each { quarter, x, y }. The scorecard shows a "Show history" toggle only when history is non-empty. In HISTORY MODE: all other dots hide; the selected stakeholder shows its current dot plus one dashed "history dot" per snapshot, placed via coordToPct(h.x, h.y) and labelled with h.quarter; a dashed TRAIL connects the snapshots in order ending at the current position (drawn in a 0..100 viewBox where cx = ((x+10)/20)*100, cy = ((10-y)/20)*100 — the same transform). Selecting any dot exits history mode. (How snapshots are CREATED — a quarterly job copying the live weighted position into history — is a backend concern captured later; on the client, history is read from the stakeholder record.)
...
MUI BUILD MAP — stage: a Box using CSS grid (6x4) for the zone cells, each cell a Paper/Box themed by the zone color (zone color/text pulled from the theme-mapped STATUSES, no inline hacks). Dots layer: a positioned Box; each dot a ButtonBase (for click/double-click/keyboard) wrapping an Avatar/Box circle, absolutely positioned by the coordToPct percentages, with a Tooltip; dragging via pointer-event handlers on the ButtonBase (MUI composition, not a third-party DnD lib). Count badge: Badge or a small Chip. Axis ticks & legend: Typography. History trail: an SVG overlay (allowed as content, not a UI primitive) or successive positioned Boxes. Scorecard: a Drawer (persistent, right) or a Paper side panel; rows as Stack + Typography; status as Chip (lg); tags/issues as Chip; owners as AvatarGroup; collapse via IconButton (ChevronLeft/Right). Selection state lifts to the page so Scoring/Lists/Map share the selected stakeholder.` },
