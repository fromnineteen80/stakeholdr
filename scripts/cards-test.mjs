// Phase 21 — CARD-CONSISTENCY PROBE (the card-anatomy visual-gate rule).
...
async function probe(label, navText, shot) {
  ...
  console.log(`CARDS ${label}: grids [${gridSummary}] chips [${[...new Set(m.chips)].join(', ')}] title-dx [${[...new Set(m.titles)].join(', ')}]`);
}
...
console.log('card-consistency probe: PASS (uniform heights · one pill scale · flush titles)');
