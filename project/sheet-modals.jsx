...
function StakeholderModal({ users, workspaces, isMaster, currentUser, activeWorkspaceLabel, existing, onCancel, onSubmit, onDelete, initialView, companyIssues, companyTags, community, stakeholders, scores, team, getWorkspacesForStakeholder, onOpenWorkspace, updateCommunityApp }) {
          ...
          <div className="sh-form-row sh-form-row-3">
            ...
            <label className="login-field"><span className="lbl">Phone</span>
              <input
                ...
                onBlur={e => set("phone", window.formatPhone(e.target.value))}
...
Object.assign(window, { StakeholderModal });

// ── FilterPopover ─────────────────────────────────────────────────
function FilterPopover({ filters, options, users, onToggle, onClear, onClose }) {
...
// ── SortPopover ─────────────────────────────────────────────────
function SortPopover({ fields, sortKey, sortDir, setSortKey, setSortDir, onClear, onClose }) {
  const wrapRef = useRef(null);
  useEffect(() => {
    function onDoc(e) {
      if (!wrapRef.current?.contains(e.target) && !e.target.closest(".filter-button-wrap")) onClose();
...
function IssueSelector({ selected, companyIssues, onChange, restrict }) {
  const [draft, setDraft] = useState("");
...
Object.assign(window, { IssueSelector });
...
function NotesModal({ stakeholder, users, currentUser, onClose, onAdd }) {
...
Object.assign(window, { NotesModal });
