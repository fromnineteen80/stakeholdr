...
window.Store = (() => {  const PREFIX = "hpsm:";        // localStorage namespace
...
window.uid = function (prefix) {  const u = (window.crypto && window.crypto.randomUUID)
...
window.nowStamp = function () { return new Date().toISOString(); };

// Platform-aware command-palette shortcut label (Mac vs PC).
window.cmdKeyLabel = (typeof navigator !== "undefined" && /Mac|iPhone|iPad|iPod/.test(navigator.platform || navigator.userAgent)) ? "⌘K" : "Ctrl K";

// React hook: state that is persisted AND kept in sync across tabs/users.
// Drop-in replacement for useState(seed).
window.usePersistentState = function usePersistentState(table, seed) {
  const { useState, useRef, useEffect } = React;
  const [value, setValue] = useState(() => window.Store.load(table, seed));
  ...
  useEffect(() => {
    ...
    window.Store.save(table, value);
  }, [value]);

  useEffect(() => {
    return window.Store.subscribe(table, (incoming) => {
