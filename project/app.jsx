...
function AppLoggedIn({ currentUser, users, setUsers, persistUsers, logOut, setCurrentUser }) {
  ...
  function updateStakeholder(id, patch) {
    setStakeholders(prev => prev.map(s => s.id === id ? { ...s, ...patch, updatedAt: window.nowStamp() } : s));
  }
  function updateScore(shId, tmId, val) {
    setScores(prev => {
      const existing = (prev[shId] || {})[tmId];
      const now = window.nowStamp();
      const merged = {
        x: val.x, y: val.y,
  ...
  function updateTeam(tmId, patch) {
    setTeam(prev => prev.map(m => m.id === tmId ? { ...m, ...patch, updatedAt: window.nowStamp() } : m));
  }
  function addTeamMember(userId) {
    if (!userId) return;
    // Guard: skip if this user is already on the team.
    if (team.find(m => m.userId === userId)) return;
    const id = window.uid("tm");
    setTeam(prev => [...prev, { id, userId, weight: 1.0, createdAt: window.nowStamp(), updatedAt: window.nowStamp() }]);
  }
  ...
  function updateWorkspace(wsId, patch) {
    setWorkspaces(prev => prev.map(w => w.id === wsId ? { ...w, ...patch, updatedAt: window.nowStamp() } : w));
  }
  ...
  function updateCommunityApp(app) {
    const stamped = { ...app, updatedAt: window.nowStamp() };
    setCommunity(prev => {
  ...
  function addStakeholder(data, forceWorkspaceId) {
    ...
    setMessages(prev => ({
      ...
      ["c-system"]: [
        ...(prev["c-system"] || []),
        { id: window.uid("m"), from: "u-system", body: text, at: new Date().toISOString(), kind: "scoring-needed" }
      ]
    }));
    return id;
  }
    ...
    setScores(prev => {
      ...
      for (const m of team) {
        ...
        next[id][m.id] = {
          ...
          createdAt: sc.createdAt || window.nowStamp(),
    ...
    <div className="app">
      ...
      <div className="ws-tab-bar">
        ...
        <div className="ws-tab-meta">
          <span>Updated {window.formatDateLong ? window.formatDateLong(
            (isMaster
      ...
      {scoringProfileShId && window.StakeholderModal && (() => {
      ...
      <EditProfileModal
        ...
        onSave={(next) => {
          ...
          setUsers(prev => prev.map(u => u.id === next.id ? { ...u, ...next, updatedAt: window.nowStamp() } : u));
        ...
        {detailStakeholder && (
          <>
            <div className="drawer-head">
              <h2>{window.displayName(detailStakeholder) || detailStakeholder.name}</h2>
