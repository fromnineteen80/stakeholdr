...
const { useState: usePlanState } = React;
...
function PlanView({
  ...
  const [openId, setOpenId] = usePlanState(null);
  const [mode, setMode] = usePlanState("edit"); // edit | review
  React.useEffect(() => {
...
function today() { return window.nowStamp ? window.nowStamp() : new Date().toISOString(); }
function planDate(d) { return window.formatDateLong ? window.formatDateLong(d) : d; }
...
function PlanHome({ explainerSlot, plans, workspaces, wsStakeholders, onOpen, onReview, onNew, onDelete, currentUser, users }) {
  const D = window.STAKEHOLDER_DATA;
  const [search, setSearch] = usePlanState("");
  ...
  const cols = [
    ...
    { key: "site", label: "Site", w: "minmax(120px,1fr)", render: p => p.site && window.STAKEHOLDER_DATA.SITES ? (window.STAKEHOLDER_DATA.siteLabel(window.STAKEHOLDER_DATA.SITES.find(s => s.id === p.site) || {}) || "-") : "-" },
    { key: "status", label: "Status", w: "120px", render: p => <span className="comm-stage-text" style={{ color: (window.PLAN_STAGE_FG && window.PLAN_STAGE_FG[p.status]) || "var(--ink-2)" }}>{p.status || "Idea"}</span> }
  ];
    ...
    return (
            <div className="comm-card plan-card" key={p.id}>
              ...
              <div className="comm-card-badges">
                <span className="tag" style={(window.PLAN_GOAL_COLORS && window.PLAN_GOAL_COLORS[p.goalModel]) ? { background: window.PLAN_GOAL_COLORS[p.goalModel].bg, color: window.PLAN_GOAL_COLORS[p.goalModel].fg, borderColor: "transparent" } : { background: "var(--bg-2)", color: "var(--ink-2)" }}>{goalName(p.goalModel)}</span>
                <span className="spacer" style={{ flex: 1 }} />
                <span className="comm-stage-text" style={{ color: (window.PLAN_STAGE_FG && window.PLAN_STAGE_FG[p.status]) || "var(--ink-2)" }}>{p.status || "Idea"}</span>
              </div>
              {p.summary ? (
...
function PlanEditor({ plan, workspace, workspaces, stakeholders, allStakeholders, scores, team, isMaster, stakeholderWorkspaces, setStakeholderWorkspaces, addStakeholder, updateStakeholder, getWorkspacesForStakeholder, updateCommunityApp, onOpenWorkspace, users, community, companyIssues, currentUser, updatePlan, onBack, onReview }) {
  const D = window.STAKEHOLDER_DATA;
  const p = plan;
  const [newSh, setNewSh] = usePlanState(false);
  const [addExisting, setAddExisting] = usePlanState(false);
  const [viewShId, setViewShId] = usePlanState(null);
  ...
  const StakeholderModal = window.StakeholderModal;
  const StakeholderProfile = window.StakeholderProfile;
  ...
  const ranked = (stakeholders || []).map(s => ({
    s,
    suggestion: window.sepScore(s, sector, goal, sepCtx),
            ...
            <div className="plan-sh-table">
              ...
              {ranked.map(({ s, suggestion, override }) => (
                <div className="plan-sh-trow" key={s.id} onClick={() => setViewShId(s.id)} title="Open stakeholder">
                  <span className="plan-sh-name">{window.displayName ? (window.displayName(s) || s.name) : s.name}</span>
              ...
              <div className="plan-sh-addrow">
                <PlanAutocomplete
                  ...
                  getLabel={s => (window.displayName ? (window.displayName(s) || s.name) : s.name)}
                  getSub={s => s.type}
...
function LeadPick({ users, value, onChange }) {
  const [open, setOpen] = usePlanState(false);
...
function PlanReview({ plan, workspace, stakeholders, users, community, companyIssues, currentUser, scores, team, onBack, onEdit }) {
  ...
  const rankedSh = (stakeholders || []).map(s => {
    const sug = window.sepScore(s, sector, goal, sepCtx);
      ...
      <div className="plan-review-body">
        ...
        <header className="plan-review-head">
            ...
            <div className="plan-review-models">
              {(window.PLAN_GOAL_COLORS && window.PLAN_GOAL_COLORS[p.goalModel])
                ? <span className="tag" style={{ background: window.PLAN_GOAL_COLORS[p.goalModel].bg, color: window.PLAN_GOAL_COLORS[p.goalModel].fg, borderColor: "transparent" }}>{goal.name}</span>
          ...
          <RS title="Community Investment">
            {linkedCommunity.length ? linkedCommunity.map(c => (
              <div className="plan-review-comm" key={c.id}>
                ...
                <span className="muted"> - {c.kind} · {c.stage} · {window.communityEntryAmount ? window.communityEntryAmount(c) : ""}</span>
              </div>
...
function PlanList({ items, onChange, placeholder }) {
  const [draft, setDraft] = usePlanState("");
...
function PlanTeam({ team, users, onChange }) {
  ...
  const [adding, setAdding] = usePlanState(false);
  return (
...
function PlanStrategies({ strategies, users, onChange }) {
  function add() { onChange([...(strategies || []), { id: window.uid("st"), title: "", how: "", timing: "", ownerId: "" }]); }
      ...
      {linked.map(c => (
        <div className="plan-comm-row" key={c.id}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div className="muted" style={{ fontSize: 11 }}>{c.kind} · {c.stage} · {window.communityEntryAmount ? window.communityEntryAmount(c) : ""}</div>
          </div>
...
const PLAN_STAGES = ["Idea", "Proposed", "Under Review", "Active", "Complete"];
// Plan-type (goal model) pill colors - same warm palette family as community KindBadge.
window.PLAN_GOAL_COLORS = {
...
window.sepScore = function (s, sector, goal, ctx) {
  const D = window.STAKEHOLDER_DATA;
  ...
  const issueRel = pIssues.length
    ...
    : 0.5;
  const affil = window.affiliatedCommunity ? window.affiliatedCommunity(s.id, ctx.community || []) : [];
