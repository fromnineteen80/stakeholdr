...
function Avatar({ user, size = 28, ring = false, online = false, onClick, title }) {
  if (!user) return null;
...
function UserStack({ users, currentUserId, max = 3, onClick, size = 28 }) {
...
function UserListPopup({ open, onClose, users, currentUserId, onMessage }) {
  const others = users.filter(u => u.id !== currentUserId && u.role !== "system");
  return (
    <>
...
function ProfileMenu({ open, anchor, user, onClose, onEditProfile, onMessages, onSettings, onLogOut, isManager }) {
  if (!open) return null;
  return (
...
function LoginView({ onLogin }) {
  ...
  const loginBrand = (() => {
    ...
    try {
      const cfg = window.Store && window.Store.load("appConfig", null);
  ...
  function submit(e) {
    ...
    const u = {
      ...
      avatarUrl: photo,
      presence: "online",
      createdAt: window.nowStamp(),
      updatedAt: window.nowStamp()
    };
    onLogin(u);
  }
      ...
      <div className="login-card">
        ...
        <div className="login-sample">
          ...
          <div className="login-sample-row">
            {window.STAKEHOLDER_DATA.USERS.slice(0, 5).map(u => (
...
// Tiny manager star.
function ManagerStar({ size = 12, title }) {
  return (
    <span
...
function UserName({ user, size = 12 }) {
  if (!user) return null;
...
function EditProfileModal({ open, user, onClose, onSave, companyFunctions }) {
        ...
        <div className="modal-body">
          ...
          <div className="login-field"><span className="lbl">Markets</span>
            <div className="profile-chip-pick">
              {Object.keys(window.STAKEHOLDER_DATA.MARKETS).map(m => (
          ...
          {(draft.markets || []).length > 0 && (
            <div className="login-field"><span className="lbl">Regions</span>
              <div className="profile-chip-pick">
                {(draft.markets || []).flatMap(m => (window.STAKEHOLDER_DATA.MARKETS[m] || [])).map(r => (
...
function UserAutocomplete({ users, value, onChange, placeholder, clearable, autoFocus, showAvatar }) {
...
function MultiOwnerPicker({ users, owners, onChange, size = 26 }) {
...
// ── ConfirmDialog ─────────────────────────────────────────────────────
function ConfirmDialog({ open, title, body, confirmLabel, cancelLabel, danger, onConfirm, onCancel }) {
  if (!open) return null;
  return (
    <>
...
// (not edit them).
function OwnersDisplay({ users, owners, size = 22, label = "owners" }) {
  const [open, setOpen] = useState(false);
  const wrapRef = useRef(null);
  useEffect(() => {
    if (!open) return;
...
Object.assign(window, { Avatar, UserStack, UserListPopup, ProfileMenu, LoginView, EditProfileModal, OwnerPicker, UserAutocomplete, MultiOwnerPicker, ConfirmDialog, OwnersDisplay, ManagerStar, ManagerBadge, UserName });
