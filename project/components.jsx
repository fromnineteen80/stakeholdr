...
function StatusPill({ status, size = "sm" }) {
  const meta = window.STAKEHOLDER_DATA.STATUSES[status];
  if (!meta) return null;
  return (
    <span
      className="pill"
...
function coordToPct(x, y) {
  const cx = ((x + 10) / 20) * 100;
  const cy = ((10 - y) / 20) * 100;
  return { left: cx + "%", top: cy + "%" };
}
function pctToCoord(leftPct, topPct) {
...
function FilterSection({ label, values, active, onToggle, render }) {
...
function SortFieldList({ fields, sortKey, sortDir, onSet }) {
...
Object.assign(window, { StatusPill, PriorityPill, StatusDot, Tags, Icon, coordToPct, pctToCoord, abbrev, abbrevTitle, displayName, normalizeUrl, formatPhone, formatDateLong, FilterSection, SortFieldList });
