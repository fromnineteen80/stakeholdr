...
function StakeholderPicker({ stakeholders, selected, onChange, placeholder }) {
  ...
  const matches = stakeholders
    ...
    .filter(s => !q || (window.displayName(s) || s.name || "").toLowerCase().includes(q) || (s.org || "").toLowerCase().includes(q))
    .slice(0, 8);
        ...
        <div className="sh-picker-menu">
          {matches.map(s => (
            <button key={s.id} type="button" className="sh-picker-row" onMouseDown={(e) => { e.preventDefault(); onChange([...selected, s.id]); setQuery(""); }}>
              <span style={{ fontWeight: 500, fontSize: 12.5 }}>{window.displayName(s) || s.name}</span>
...
function CommunityModal({ existing, users, stakeholders, currentUser, companyIssues, onCancel, onSubmit, initialView, onOpenStakeholder, asPage }) {
  ...
  const vs = window.communityValueScore(d);
          ...
          <label className="login-field"><span className="lbl">Stakeholder / Organization Targeted</span>
            <Designed inline>
              <select value={d.representedStakeholderId} onChange={e => set("representedStakeholderId", e.target.value)}>
                ...
                {stakeholders.map(s => <option key={s.id} value={s.id}>{window.displayName(s) || s.name}</option>)}
              </select>
            </Designed>
          </label>
...
Object.assign(window, { CommunityModal });
