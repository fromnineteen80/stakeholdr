// profiles.jsx - shared read-only StakeholderProfile. Opened from the List
// (edit card "View Stakeholder") and from the Community page (entry card
// stakeholder pills). Self-contained drill: clicking an affiliated entry opens
// the read-only CommunityProfile; clicking a stakeholder there swaps the
// subject - the back-and-forth bridge between projects and stakeholders.
...
function StakeholderProfile({
  ...
  React.useEffect(() => { setSubject(stakeholder); setEntryId(null); setShowAllEntries(false); }, [stakeholder && stakeholder.id]);
  if (!subject) return null;

  const s = subject;
  const affil = window.affiliatedCommunity ? window.affiliatedCommunity(s.id, community || []) : [];
  const cumulative = window.stakeholderCumulativeUSD ? window.stakeholderCumulativeUSD(s.id, community || []) : 0;
      ...
      <div className="modal stakeholder-modal profile-modal">
        <div className="prof-header">
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div className="prof-title">{window.displayName(s) || s.name}</div>
        ...
        <div className="modal-body prof-body">
          ...
          <div className="prof-grid">
            ...
            <PRow k="Last contact">{s.lastContact ? window.formatDateLong(s.lastContact) : "-"}</PRow>
          </div>
          ...
          <div className="prof-grid">
            <PRow k="Website">{s.url ? <a className="plain-link" href={window.normalizeUrl(s.url)} target="_blank" rel="noopener noreferrer">{s.url}</a> : "-"}</PRow>
            <PRow k="Email">{s.email ? <a className="plain-link" href={"mailto:" + s.email}>{s.email}</a> : "-"}</PRow>
            <PRow k="Phone">{s.phone ? window.formatPhone(s.phone) : "-"}</PRow>
            ...
            <div className="profile-entry-list">
              {affil.slice(0, 5).map(a => (
                <button key={a.id} className="profile-entry" onClick={() => setEntryId(a.id)} title="Open application">
                  ...
                  <span className="profile-entry-amt">{window.communityEntryAmount(a)}</span>
                ...
                return (
                  <div key={n.id} className="prof-note">
                    <div className="prof-note-meta">
                      <span style={{ fontFamily: "var(--mono)", color: "var(--ink-2)" }}>{n.at ? window.formatDateLong(n.at) : ""}</span>
          ...
          <div className="modal" style={{ width: 460, maxWidth: "calc(100% - 32px)" }}>
            <div className="modal-head">
              <div style={{ minWidth: 0 }}>
                ...
                <h2 style={{ margin: 0 }}>{window.displayName(s) || s.name}</h2>
              </div>
...
Object.assign(window, { StakeholderProfile });
